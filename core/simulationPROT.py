...
def _tick_one_genome(
    ...
    # 11. Protein decay + synthesis  (moved before step 9)
    # gene_prot is now available for the status threshold check below.
    # Protein is still synthesised from prev_expr (last tick's mRNA) —
    # the one-tick translation delay is preserved.
    gene_prot: Dict[str, float] = {}
    for g in gene_list:
    ...
    # Activation threshold is now checked against gene_prot (protein level)
    # instead of new_expr (mRNA level).  A gene becomes "active" only when
    # enough protein has accumulated — closer to real transcription/translation.
    ...
    for g, (thr, mode, prom, enh, size_kDa, on_cond, off_cond, func) in all_genes.items():
        if deact_delay[g]:
            ...
            new_status[g] = (
                "active"
                if (not had_off[g] and cond_all(on_cond, local) and gene_prot[g] >= thr)
                else "inactive"
            )
...
def run_simulation(
    ...
    for tick in range(ticks):
        ...
        for gobj in list(live_genomes.values()):
            ...
            can_mutate = mutation_chance > 0 and total_live < n_genomes_allowed
